...
if 'group_link' not in column_names:
    try:
        ...
        print("Column group_link added successfully!")
        conn.commit()
...
if 'last_message_id' not in column_names:
    try:
        ...
        print("Column last_message_id added successfully!")
        conn.commit()
    except sqlite3.OperationalError as e:
        print(f"Unexpected error: {e}")
...
async def process_last_messages(group_link):
    ...
    messages = [
        {"role": "system", "content": "Анализировать текст из чата и выдавать конкретную информацию интервально, Основная мысль и пункты.."},
...
async def show_subscriptions(message: types.Message):
    ...
    sent_message = await message.answer("Вы подписаны на следующие чаты,\nсообщения приходят каждые 6 часов...\n", reply_markup=markup)
...
async def periodic_task():
    ...
    while True:
        ...
        # После выполнения всех действий ждем 1 час
